...
// Changes a vector of strings to a character array
char** vector_to_array(vector<string> commands){

	string new_string;
	string last_token = commands[commands.size()-1];
	char last_char = last_token[last_token.size()-1];
	
	if(last_token == "&" && commands.size() > 1){
		amp_flag = 1;
		commands.pop_back();
	}
	...
	if(last_char == '&'){
		...
		for(int i=0;i< last_token.size()-1;i++){
			new_string += last_token[i];
		}
		commands[commands.size()-1] = new_string;
	}
	
	char* commands_array[128];
	
	for(int i = 0; i < commands.size(); i++){
		commands_array[i] = new char[commands[i].size() + 1];
		strcpy(commands_array[i], (char*)commands[i].c_str());
	}
	
	commands_array[commands.size()] = NULL;

	return commands_array;
}

// Forks and processes external commands according to the users input
int fork(char** command_array){
  ...
  if (pid < 0){
	...
	flag = execvp((const char*) command_array[0], command_array);
	executed_flag = 1;
	if(flag == -1){
		cout << "Invalid external command" <<endl;
	}
    exit( flag );
...
int main(int argc, char* argv[],char* env[]){
	...
	// Put all environment variables into a map of string strings
	...
	while(exit_flag){
		...
		// Processes command !N where N is the number of a command in the commandlist, it then executes that command from the list
			...
			if(hist_index > history_list.size()){
				...
				cout << "Clist does not start indexing at zero" <<endl;
			}
			else{
				string temp_string;
		...
		// Tells the user if tokens were invalid and adds invalid commands to the history list
			...
			history_list.push_back(tmp);
			auto commands_arr = vector_to_array(command_list);
			if(fork(commands_arr)==0){
